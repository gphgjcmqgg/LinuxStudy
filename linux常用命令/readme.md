...
find命令用于在系统中搜索符合条件的文件名(完全匹配)，如果要模糊查询，则使用通配符进行匹配,通配符是完全匹配的
通配符：
1. ?          匹配一个任意字符
2. *          匹配任何内容
3. []         匹配括号中任意一个字符      [abc]
4. [-]        匹配括号中任意一个字符，-表示范围   [A-Za-z] , [0-9]
5. [^]        逻辑非，表示匹配不是中括号内的一个字符      [^0-9]
find . -name "a?c"

grep命令用于在文件中搜索符合条件的字符串（包含匹配），如果要模糊查询，则使用正则表达式进行匹配，正则表达式是包含匹配
